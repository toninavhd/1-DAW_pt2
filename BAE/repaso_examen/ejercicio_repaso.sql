...
-- 1.- Crear una vista que muestre los títulos de los discos que hayan recibido alguna puntuación y el nombre del intérprete .

CREATE VIEW DiscoPuntuado AS
SELECT D.Titulo, I.Interprete
FROM disco as Discs
INNER JOIN Interprete I ON D.IdInterprete = I.IdInterprete
INNER JOIN Puntuacion P ON D.IdDisco = P.iddisco
GO

SELECT Interprete, COUNT(Titulo) AS CantidadDiscos
FROM DiscoPuntuado
GROUP BY Interprete
GO
/*2 Hacer un procedimiento almacenado al que le pasemos como parámetros iddisco,idcliente y puntuacion 
que componen el registro de la tabla puntuacion y valide que la puntuación está entre 0 y 10, 
que iddisco es de un disco existente y que idcliente es de un cliente existente. 
Si todo es correcto insertará el registro, tomando como fecha la fecha actual 
y como id uno más que el valor máximo de id de la tabla puntuacion. (2,5 puntos)*/

CREATE PROCEDURE puntuacion_procedure (
  @iddisco INT, 
  @idcliente INT, 
  @puntuacion INT)
  AS
  IF @puntuacion BETWEEN 0 AND 10 AND
  EXISTS (SELECT 1 FROM disco WHERE iddisco = @iddisco) AND
  EXISTS (SELECT 1 FROM cliente WHERE id = @idcliente)
  BEGIN
  INSERT INTO Puntuacion (Id, Idcliente, iddisco, Puntuacion, Fecha)
  VALUES ((SELECT MAX(Id) + 1 FROM Puntuacion), @idcliente, @iddisco, @puntuacion, GETDATE())
  END
GO

EXEC puntuacion_procedure @iddisco = 1, @idcliente = 1, @puntuacion = 5

SELECT * FROM Puntuacion ORDER BY Id DESC

/*Crear una tabla con un único campo llamado t de tipo varchar(100).
Insertar en la tabla creada los valores 'Cliente', 'Puntuacion', 'Disco' e Interprete'.
Hacer un procedimiento almacenado que contenga un cursor que recorra la tabla
creada y nos muestre el número de registros de la tabla con el nombre leído.
Hacer una prueba de ejecución. (3 puntos)*/

CREATE TABLE NombreTabla (t VARCHAR(100));
INSERT INTO NombreTabla (t) VALUES ('Cliente'), ('Puntuacion'), ('Disco'), ('Interprete');
GO

CREATE PROCEDURE contar_registros AS
DECLARE @nombre VARCHAR(100);
DECLARE cursor_nombre CURSOR FOR SELECT t FROM NombreTabla;
GO

EXEC contar_registros 
GO
OPEN cursor_nombre;
FETCH NEXT FROM cursor_nombre INTO @nombre;
