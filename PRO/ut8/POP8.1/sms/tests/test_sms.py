...
if os.path.exists('solution.py'):
    from solution import SIM, SMS, DbHandler, SMSError
else:
    from main import SIM, SMS, DbHandler, SMSError
...
def test_send_sms_from_sim_fails_when_locked(sim1: SIM, db_con: sqlite3.Connection):
    RECIPIENT = '699123456'
    MESSAGE = 'Python is cool!'

    with pytest.raises(SMSError) as err:
        sim1.send_sms(recipient=RECIPIENT, message=MESSAGE)
    assert str(err.value) == 'SMS is locked'
...
def test_send_sms_from_sim_fails_when_invalid_recipient_phone_format(
    sim1: SIM, db_con: sqlite3.Connection
):
    ...
    for recipient in RECIPIENTS:
        with pytest.raises(SMSError) as err:
            sim1.send_sms(recipient=recipient, message=MESSAGE)
        assert str(err.value) == 'Recipient has invalid phone format'
