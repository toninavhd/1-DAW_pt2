...
if os.path.exists('solution.py'):
    from solution import DbHandler, Mail, MailError, MailServer
else:
    from main import DbHandler, Mail, MailError, MailServer
...
def test_send_mail_from_mailserver_fails_when_not_logged_in(mailserver1: MailServer):
    ...
    assert str(err.value) == 'User is not logged in'
...
def test_send_mail_from_mailserver_fails_when_invalid_recipient_email_format(
    ...
    for recipient in RECIPIENTS:
        ...
        assert str(err.value) == 'Recipient has invalid mail format'
